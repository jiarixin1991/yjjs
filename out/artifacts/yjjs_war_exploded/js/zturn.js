 (function(win,doc,undefined){
 	var zturn=function(turn){
 		...
 		this.num_li=this.zturnitem.length   //轮播元素个数
 		this.zturnPy=turn.Awidth/(this.num_li-1)  //zturnPy为每个的偏移量
 		this.init()
 		this.turn_()
 		this.auto_play()
 		return this
 		...
 		constructor:zturn,
 		 init:function(){
				...
				this.zturn.children(".zturn-item").each(function(index,element){	 		
						//index是第几个元素 X是选取的中间数 num_li是总数     element是每个元素的内容
						var rt=1//1:右侧：-1：左侧
						...
						var marLeft = 0;                //向左或向右的偏移量
						if(_self.num_li == 1){
							marLeft = -_self.turn.width/2;
							//console.log("偏移量1："+marLeft);
						} else {
							marLeft = -_self.turn.width/2+_self.zturnPy*rt*i
							//console.log("偏移量2："+marLeft);
						}
						$(this).css({
							'position':'absolute',
							'left': '50%',
 		 ...
 		 turn_:function(){
 		 	var _self=this
 		 	this.zturnitem.click(function(){	 	   //点击模块改变中间数X
